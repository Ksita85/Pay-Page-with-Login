...
class ResetPasswords{
    ...
    public function __construct(){
        ...
        $this->mail->Port = 2525;
        $this->mail->Username = '3b291d4a3417d8';
        $this->mail->Password = '4daa39f4374e41';
    }
    ...
    public function resetPassword(){
        ...
        if (empty($_POST['pwd'] || $_POST['pwd-repeat'])){
            flash("newReset", "Please fill out all fiedls");
            redirect($url);
        }else if($data['pwd'] != $data['pwd-repeat']){
            flash("newReset", "Passwords don't match");
            redirect($url);
        ...
        if(!$tokenCheck){
            flash("newReset", "You need to re-Submit your request");
            redirect($url);
        }

        $tokenEmail = $row->pwdResetEmail;
        if(!$this->userModel->findUserByEmailOrUsername($tokenEmail, $tokenEmail)){
            flash("newReset", "User doesn't exist");
            redirect($url);
        }
        ...
        flash("newReset", "Password Updated");
        redirect($url);
